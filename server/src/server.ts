...
app.get(
  "/api/publishers",
  async (
    req: Request<object, PaginatedResponse<Publisher> | { error: string }, unknown, { search?: string; isActive?: string; page?: string; limit?: string }>,
    res: Response<PaginatedResponse<Publisher> | { error: string }>
  ): Promise<void> => {
    try {
...
app.get(
  ...
  async (
    req: Request<{ publisherId: string }>,
    res: Response<Publisher | { error: string }>
  ): Promise<void> => {
    try {
      const { publisherId }: { publisherId: string } = req.params;
...
app.put(
  ...
  async (
    ...
    try {
      const { publisherId }: { publisherId: string } = req.params;
      const bodyContent: Publisher = req.body as Publisher;
...
app.get(
  "/api/publishers/:publisherId/audit-logs",
  async (
    req: Request<{ publisherId: string }, PaginatedResponse<AuditLog> | { error: string }, unknown, { page?: string; limit?: string }>,
    res: Response<PaginatedResponse<AuditLog> | { error: string }>
  ): Promise<void> => {
    try {
